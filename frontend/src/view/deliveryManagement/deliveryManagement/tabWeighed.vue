<template>
  ...
  <div class="mt-5" :style="{ height: cardHeight }">
    <vxe-table :loading="data.loading" :column-config="{ minWidth: '120px' }" :row-config="{ keyField: 'id' }"
      :data="data.tableData" :height="tableHeight" align="center">
...
import type { DispatchOrderSummary, DispatchPackingTask, DispatchSourceDecision } from '@/types/DeliveryManagement/DispatchWorkflow'
...
const data = reactive({ keyword: '', group_id: null as number | null, member_id: null as number | null,
  loading: false, tableData: [] as DispatchOrderSummary[], total: 0, pageIndex: 1, pageSize: DEFAULT_PAGE_SIZE,
