...
- 分派有界、只读的文本、代码、日志阅读、搜索、提取、枚举或高批量阅读任务时，应优先并尽可能使用本机实际安装的 `v4_flash_worker`；不得假设或填写未安装的其他版本名称。
- 使用 `v4_flash_worker` 前必须先加载 `$use-v4-flash-worker`，通过已安装的 plaintext Hook 暂存完整任务，并以 `fork_turns="none"` 启动；不得使用继承上下文、消息直投、直接 API、其他 Codex CLI、其他 provider 或应用作为替代路径。
- plaintext Hook 或 `v4_flash_worker` 不可用时，必须报告明确的失败边界并停止该分派，不得静默切换模型或调用路径。
