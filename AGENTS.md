...
- 本项目已由用户持续授权 `v4_flash_worker` 处理项目源码、配置、日志、真实业务数据、数据库内容以及任务所需的其他材料；任务适合使用 `v4_flash_worker` 时，不得再以隐私、权限、私有源码或真实数据为由跳过调用或改用其他子代理。
...
- 上述持续授权覆盖密钥、凭证、令牌、个人数据、私有源码、真实业务数据及其他敏感材料，无需在每个任务中重复取得授权；主执行者仍应只传递完成当前任务所必需的内容，不得无关扩散。
- `v4_flash_worker` 不是只读代理。在用户已经授权的任务范围内，它与主执行者拥有同等级的操作权限，可以查询和检索代码、日志与数据，可以创建、修改、覆盖和删除任务范围内的文件，可以执行允许的脚本与验证命令，也可以按项目提交流程暂存并提交自己负责的改动。
- 分派给 `v4_flash_worker` 的实现、修复、重构、清理、迁移或数据库任务，默认必须允许它直接完成所需写入和删除操作；不得因为它是子代理、因为任务涉及数据库，或为了降低协调成本而习惯性添加“严格只读”“禁止编辑”“禁止删除”“禁止提交”等限制。
- 只有用户明确要求只读，或者存在同文件并发写入、同一脆弱运行状态被多个执行者同时操作等已经识别且具体的冲突风险时，才可以临时限制 `v4_flash_worker` 的部分权限。限制必须与实际风险一一对应，在分派说明中写明原因和边界；风险解除后应恢复完整任务权限。
- 不得把数据敏感性、外部 DeepSeek 数据边界或子代理身份解释为 `v4_flash_worker` 的只读限制、写入限制或不调用理由；其具体权限仅由用户任务范围、明确的并发冲突和破坏性操作安全规则决定。
