...
public partial class ErpPendingReceiptService
{
        ...
        DateTime now)
    {
        ...
        if (requested.Count == 0)
        {
            ...
            if (defaultArea == null)
            {
                throw new InvalidOperationException($"商品 {product.sku} 未选择库区，且采购人所属小组未绑定有效库区");
            }
            requested =
            [
        ...
        {
            if (allocation.warehouse_area_id <= 0 || allocation.qty <= 0
                || allocation.qty > inboundQty - allocatedQty)
            {
        ...
        {
            throw new InvalidOperationException($"商品 {product.sku} 的库区/所属人分配数量必须大于 0，且合计等于实际入库数量 {inboundQty}");
        }
        ...
        foreach (var allocation in requested)
        {
            var area = await EnsureReceiptAreaLocationAsync(
                shipment,
                product,
                currentUser,
                now,
        ...
        long tenantId)
    {
        ...
        await using var command = CreateCommand(
            ...
            SELECT receipt_item_id,warehouse_area_id,warehouse_area_name,
                   goods_owner_id,goods_owner_name,qty
        ...
        while (await reader.ReadAsync())
        {
            ...
            {
                warehouse_area_id = reader.GetInt32(1),
                warehouse_area_name = reader.GetString(2),
                goods_owner_id = reader.GetInt32(3),
                goods_owner_name = reader.GetString(4),
